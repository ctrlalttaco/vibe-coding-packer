...
def k8sVersions = ['1.29', '1.30', '1.31', '1.32', '1.33']
def instanceTypeOverrides = ['c5.large', 'c5.xlarge', 'c5.2xlarge', 'c6g.large', 'c6g.xlarge', 'c6g.2xlarge']
...
pipeline {
    ...
    parameters {
        ...
        choice(name: 'K8S_VERSION', choices: ['all'] + k8sVersions, description: 'Kubernetes version to build (leave empty for all)')
        ...
        choice(name: 'INSTANCE_TYPE_OVERRIDE', choices: ['all'] + instanceTypeOverrides, description: 'Override instance type for builds (leave empty for auto-selection)')
    }
    triggers {
            ...
            steps {
                script {
                    ...
                    if (params.DISTRO == 'all') {
                        ...
                        filteredDistros = [params.DISTRO]
                    }

                    for (distro in filteredDistros) {
                        def isEks = distro.contains("-eks")
                        def k8sVersionsToUse = isEks ? filteredK8sVersions : [null]
                        for (arch in filteredArchitectures) {
                            for (k8sVersion in k8sVersionsToUse) {
                                // Skip non-EKS distros if k8s_version is set
                                if (!isEks && k8sVersion != null) {
                                    continue
                                }
                                ...
                                if (distro == 'amazon-linux-2-eks' && k8sVersion && k8sVersion > '1.32') {
                                    echo "Skipping ${distro}-${arch}-${k8sVersion} - Kubernetes version ${k8sVersion} is not supported"
                                    continue
                                }
